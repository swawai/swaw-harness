...
6. 产品全量验证是显式证据且保持独立，不进入 required Ruleset；产品工作流不得依赖治理专属源码。
7. `protect-main.json`、其中的声明名和远端身份键不得改名；改名必须作为显式迁移，不能由普通 reconcile 隐式完成。
8. Required status check 必须绑定 GitHub Actions 的 integration ID，避免同名外部 status 冒充；目标状态唯一 required context 是治理专属 `Change policy`，更换执行主体必须作为显式控制平面迁移。
...
13. 安装治理时先合入受信 policy 与候选验证，再创建治理专属 active Ruleset；从旧组合规则迁移时必须先激活只含 `Change policy` 的新规则，再从 `protect-main` 移除旧的重型 required context。
14. `Change policy` 是治理专属 Ruleset 的唯一 required context；产品与治理全量验证仅提供显式或自身信任根变更时的证据，停用与卸载不得删除其独立 workflow。
