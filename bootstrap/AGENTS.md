...
- **BOOTSTRAP-003 — 便携工具链一键编译。** Bootstrap 必须自动下载并设置便携 Rust 与 MSVC 编译环境，无需用户预装、配置或交互干预，即可一键编译出 Harness 核心。
- **BOOTSTRAP-004 — 构建环境只进入工具子进程。** `bootstrap/windows/toolchain/environment.ps1` 只生成环境计划，`bootstrap/windows/builder/process.ps1` 只把该计划注入实际工具子进程；不得修改后再恢复父 PowerShell 进程，也不得生成要求调用者 dot-source 的环境脚本，工具入口必须使用明确支持的 executable 路径。
