...
function Read-SwawHarnessWindowsBootstrapContract {
    ...
    Assert-SwawHarnessObjectFields `
        -Value $Contract `
        -Expected @('schema', 'targetId', 'rust', 'msvc', 'product') `
        -Description 'Windows Bootstrap contract'
    Assert-SwawHarnessObjectFields `
        -Value $Contract.rust `
    ...
    Assert-SwawHarnessObjectFields `
        ...
        -Description 'Windows Bootstrap product contract'
    ...
    $Package = ([string]$Contract.product.package).Trim()
    $Binary = ([string]$Contract.product.binary).Trim()
    $MsvcProductLine = ([string]$Contract.msvc.productLine).
        Trim().ToLowerInvariant()
    $MsvcLicenseAcceptance = ([string]$Contract.msvc.license.acceptance).
        Trim().ToLowerInvariant()
    if ([string]$Contract.schema -cne 'swaw.harness.bootstrap.windows/v2') {
    ...
    if ($TargetId -cne 'x86_64-pc-windows-msvc') {
        throw 'Windows Bootstrap v2 supports x86_64-pc-windows-msvc only.'
    }
    ...
    if ($Profile -cne 'minimal') {
        throw 'Windows Bootstrap v2 requires the minimal Rust profile.'
    }
    if ($MsvcProductLine -cne 'vs2026') {
        throw 'Windows Bootstrap v2 requires the VS 2026 product line.'
    }
    if ($MsvcLicenseAcceptance -cne 'by-bootstrap-invocation') {
        throw 'Windows Bootstrap MSVC license acceptance is invalid.'
    }
    if ($Package -cnotmatch '^[a-z0-9][a-z0-9_-]{0,127}$') {
        throw 'Windows Bootstrap product package is invalid.'
    }
    if ([IO.Path]::GetFileName($Binary) -cne $Binary -or
        $Binary -cnotmatch '^[A-Za-z0-9][A-Za-z0-9._+-]{0,127}\.exe$') {
        throw 'Windows Bootstrap product binary is invalid.'
    }
