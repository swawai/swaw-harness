...
- **WIN-BOOT-004 — Windows 便携工具链与 native 路径预算。** Windows Bootstrap 必须自动下载并设置便携 Rust 与 MSVC 编译环境，无需用户预装、配置或交互干预，即可编译出 Harness 核心；绝对仓库根不超过 90 字符时，交给 native 构建工具的路径不得超过 240 字符，不得通过盘符映射、reparse point、8.3 名称、环境变量替换或修改操作系统长路径设置来满足预算。
