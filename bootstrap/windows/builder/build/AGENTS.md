...
Candidate 是 Build 完成后固化在 `<repository>/data.repo/windows.build/<product>/candidates/<CandidateId>/` 中、已经验证且不可变的发布输入，目录内保存产物和 `candidate.json`。CandidateId 是该目录的名称，由 `candidate.json` 的 schema、Contract revision、PlatformTargetId、产物名称、产物长度与产物内容 SHA-256 共同确定。
