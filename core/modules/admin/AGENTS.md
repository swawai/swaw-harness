...
- **ADMIN-004 — 普通 Harness 用户创建。** 普通 Harness 用户必须绑定 `<DataHome>/<UserId>.exe`、`<DataHome>/<UserId>/` 与自己的 Core Host 版本指针；三者的原子建立与恢复、受管记录、技能图来源及其他最小目录，留待首个 `admin/user/create` 技能实现确定，旧 seed 专用实现不得作为当前能力保留。
- **ADMIN-005 — Admin 调用授权。** Core Host 如何为 Admin 管理技能证明调用具有 Admin 管理权限，以及普通 Harness 用户如何被禁止调用这些技能，留待首个管理技能实现确定；UserId、环境变量和 `admin/` 文件系统路径均不得单独作为授权凭据。
