...
- **ADMIN-002 — 固定 Admin Entry。** Admin EntryRoot 固定为 `data/admin/`，配置根固定为 `data/admin/config/`，当前 Core 配置树固定为 `data/admin/config/core/`，共享模块发布根固定为 `data/admin/modules/`，Entry 生命周期锁固定为 `data/admin/entry.lock`；Admin Entry 不使用旧 seed Runtime Release，不得整体替换或删除。
