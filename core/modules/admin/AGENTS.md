...
- **ADMIN-002 — 固定 Admin Entry。** Admin EntryRoot 固定为 `data/admin/`，技能图根固定为 `data/admin/map/`，Core 技能图固定为 `data/admin/map/core/`，共享模块发布根固定为 `data/admin/modules/`，Entry 生命周期锁固定为 `data/admin/entry.lock`；Admin Entry 不使用旧 seed Runtime Release，不得整体替换或删除。
...
- **ADMIN-004 — 普通 Entry 创建。** 普通 Entry 的最小目录、受管记录、技能图来源、launcher 安装与恢复流程，留待首个 `admin/entry/create` 技能实现确定；旧 seed 专用实现不得作为当前能力保留。
