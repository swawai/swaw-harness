...
- **CORE-MODULE-002 — 作者目录不声明运行地址。** `core/modules/` 中 Rust package、模块、函数和资产目录只按作者代码职责组织，不得使用 `swaw-harness.resource.json`、`swaw-harness.facet.json` 或目录位置声明 Runtime Core Tree 地址；运行 Resource、Facet 与 ModuleId 选择只由 `data/swaw-harness/core/` 声明。
