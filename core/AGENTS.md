...
- **CORE-010 — Help 描述不授予执行权。** 技能图根和任意技能节点可以提供可选的 `help.md`、`help.en.md`、`help.zh.md` 与 `help.zh-CN.md`；帮助文档必须使用无 BOM 的 UTF-8，纯 ASCII 内容作为有效 UTF-8 接受，首个非空行只作为技能路径列表摘要，缺失时节点仍由真实目录建立且只显示名称。`core/protocol` 必须按规范文件名、普通非 reparse 文件、有界大小和无 BOM 的 UTF-8 内容验证帮助，拒绝以保留 `[run]` 标记开头的摘要，按 `en`、`zh` 或 `zh-CN` 执行明确回退，并只把存在且完整通过解析的 `skill.toml` 标为可调用；每个显示节点必须验证自身文件，仅在请求深度内验证并遍历子目录。单次 Help 调用最多显示 4096 个节点并最多检查 4096 个目录，后者包括技能图根、解析目标 SkillPath 经过的目录及为校验深度边界节点自身文件而识别的直接子目录；任一预算超限必须在输出任何 Help stdout 前明确失败。帮助文档不得选择 Module Release 或改变分类节点性质。
...
- `core/host/` 已通过 Windows 命名管道接收一次批处理调用，并由 `core/protocol` 验证普通 Harness 用户的 `active user.json`、技能调用目标、SkillPath、技能声明、技能帮助文档和同一 DataHome 的已安装 Module Release；当前 `/.help` 按深度和语言只读输出完整技能路径列表且不建立 Run，单节点执行会建立 Run 记录和节点工作目录，`admin/user/create` 已复制普通用户的独立 Core 技能图快照。模块自身 `--help`、结构化日志、重播、技能子树执行和资源空间 Resource 验证尚未实现。
