...
- **CORE-001 — 基础与派生资源空间。** Resource 按事实来源、生命周期与写入权限分属资源空间；基础资源空间包括作者（源代码）、运行（发布）、runs（logs）与 context 模块专用上下文记录空间。跨资源空间工作必须直接选择目标资源空间；领域或用户机制可通过 export、远端 mount 等方式建立派生资源空间，但不得因此修改 Core 协议。
...
- **CORE-005 — 远端资源挂载。** 远端 Resource 通过显式 mount 进入派生资源空间后，应与本地 Resource 一样使用“资源空间根 + 相对文件系统路径”寻找并执行 Facet，不另建远端专用的寻址与操作模型；mount 的物化、同步、离线、身份和凭据边界仍需由首个真实远端实现确定。
