...
- **CORE-004 — Facet declaration 有界授予执行权。** 只有目标 `<EntryRoot>/core/` Runtime Core Tree Facet 叶目录中的 `swaw-harness.facet.json` 可以选择 Module Release executable；其 `module`、`version`、`executable` 和 `arguments` 必须精确通过协议验证，所选模块必须来自同一 DataHome 的 `<DataHome>/modules/` 已验证发布目录。其他可写 Resource 数据不得声明 executable、Facet 实现或平台能力。
