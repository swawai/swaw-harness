...
- **CORE-003 — 技能节点即调用地址。** Core 技能图中一个目录就是一个技能节点；包含 `skill.toml` 时该 SkillPath 可调用，不包含时只是分类节点。可调用节点可以继续包含子节点，不得通过通用 `execute/` 子目录重复表达可执行性，也不区分 `Operation` 与 `Projection` 两种节点类型。
