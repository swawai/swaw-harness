...
- **HarnessRoot**：包含可独立复制运行的 `data/` 的目录；在源码检出中为仓库根，复制发布后为目标位置中 `data/` 的父目录。
- **DataRoot**：可独立复制运行的 Harness 数据根，位置为 `<HarnessRoot>/data`，只保存 Entry executable、EntryRoot 及其发布运行资源，不保存仓库本地 Bootstrap 状态。
- **RepositoryDataRoot**：仅属于源码仓库、不随 DataRoot 复制发布的 Bootstrap 数据根，位置为 `<repository>/data.repo`。
...
- **REPO-005 — DataRoot 可独立复制运行。** DataRoot 必须位于 `<HarnessRoot>/data`，其 Entry executable、EntryRoot 及发布运行资源不得依赖 RepositoryDataRoot 的绝对路径；仓库本地 Bootstrap 工具链、构建、暂存、缓存、锁与日志不得写入 DataRoot。
...
- **REPO-009 — RepositoryDataRoot 仅属仓库。** RepositoryDataRoot 固定为 `<repository>/data.repo`，只保存不随 DataRoot 复制发布的仓库本地数据；各宿主平台在其中使用明确的平台领域根，不得让 RepositoryDataRoot 成为运行时依赖。
