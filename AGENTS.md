...
- **Core Host executable**：名为 `swaw-harness-core.exe`、ModuleId 固定为 `swaw/core/host` 的独立 Module Release executable；同一不可变 executable 可由多个 Harness 用户共享，但每个运行中的 Core Host 进程只服务一个 Harness 用户。它解析技能调用目标，从该用户的技能图解析 SkillPath、验证 Admin 用户共享的 Module Release、建立 Run 和运行节点目录、以批处理方式监督模块进程，并分别返回 RunId 元数据、模块 stdout、模块 stderr 与退出码。固定 Admin 用户不需要 Harness 用户记录；普通 Harness 用户只有在 `user.json` 严格有效、生命周期为 `active` 且同名用户 CLI 匹配记录的长度和 SHA-256 时才可冷启动 Host。当前 Windows 实现不监听 TCP/回环地址，只接受同一 Windows 登录会话通过本机命名管道发起的调用。
...
- **技能调用目标（Skill invocation target）**：用户 CLI 提交给 Core Host 的规范相对路径，形式为 `<SkillMapId>/<SkillPath>` 或 `<SkillMapId>/<SkillPath>/<显式 NodeMethod 选择器>`；当前只支持 SkillMapId `core`。省略选择器时使用隐式 `node` 方法执行目标技能节点；显式选择器由 Core Host 在最后一个路径段识别，不属于 SkillPath，也不对应技能图中的真实目录。
- **节点方法（Node method）**：Core Host 对技能调用目标提供的封闭控制面方法。当前方法集合是省略选择器时使用的 `node`，以及显式保留的 `/.tree`、`/.tree.parent-success`、`/.tree.no-structure`、`/.help` 和 `/.plan`；它们分别表示单节点执行、普通子树、强父子顺序子树、无父子顺序子树、能力说明和计划预检，不得通过 `.node-facet`、真实点号目录或 `skill.toml` 选择 executable。当前 Core Host 只实现 `node`，显式保留方法必须明确报告尚未实现。
...
- `data/admin.exe core/admin/user/create alice` 与随后 `data/alice.exe core/helloworld [recipient]` 已形成普通 Harness 用户创建及单节点 Run 调用闭环；这只证明 UserHome、用户 CLI、Core Host 版本指针、Core 技能图快照、单节点运行记录和工作目录可被建立、恢复与执行。结构化日志、重播、资源空间 Resource 验证、Harness 用户授权、模块安装、技能组合及用户删除、升级仍未实现，不得由该样例外推为完整系统。
