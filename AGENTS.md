...
1. `docs/swaw-harness-spec.md` 是仓库级规范与领域规范索引，目录内的 `SPEC.md` 是对应领域的规范事实源；规则正文只能由一处拥有，`AGENTS.md` 只规定维护方式，`README.md` 只做人类入口。
...
- 局部 `SPEC.md` 先说明当前目录作用，再按重要性列出最多 5 条 `Accepted` 和 3 条 `Open`；若仍不足以说明领域，应先检查代码组织和架构边界是否需要调整。
- 局部 `AGENTS.md` 先说明当前目录作用；允许与禁止的内容类型各最多列 5 种，维护问题最多分 3 类。没有真实局部差异时不得新建治理文件。
