...
- **`data.repo`**：位于 `<repository>/data.repo`、仅属于源码仓库的 Bootstrap 数据目录；不随 DataRoot 复制发布。
...
- **REPO-005 — DataRoot 可独立复制运行。** DataRoot 必须位于 `<HarnessRoot>/data`，其 Entry executable、EntryRoot 及发布运行资源不得依赖 `data.repo` 的绝对路径；仓库本地 Bootstrap 工具链、构建、暂存、缓存、锁与日志不得写入 DataRoot。
...
- **REPO-009 — `data.repo` 仅属仓库。** `data.repo` 固定为 `<repository>/data.repo`，只保存不随 DataRoot 复制发布的仓库本地数据；各宿主平台在其中使用明确的平台领域根，不得让 `data.repo` 成为运行时依赖。
