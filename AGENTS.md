...
- **用户 CLI executable**：Bootstrap Release 中通用文件名为 `user.exe`、安装后位于 `<DataHome>/<UserId>.exe` 的小型 console executable；它从自身文件名确定 UserId，读取该用户的 Core Host 版本指针，冷启动 Admin 用户共享的 Core Host Module Release，并以本机命名管道转发一次批处理技能调用。它不解析技能图或模块清单；除固定身份 `swaw/core/host` 外不得选择或启动其他模块。对已建立 Run 的调用，它把 Core Host 的独立 RunId 响应帧显示为标准错误上的 `[RUN] <RunId>`，不把该行混入模块标准输出。Windows Bootstrap 把它初始安装为 `<DataHome>/admin.exe`，Admin Core module executable 在创建普通用户时以相同字节安装对应 `<UserId>.exe`。
- **Core Host executable**：名为 `swaw-harness-core.exe`、ModuleId 固定为 `swaw/core/host` 的独立 Module Release executable；同一不可变 executable 可由多个 Harness 用户共享，但每个运行中的 Core Host 进程只服务一个 Harness 用户。它解析技能调用目标；`/.help` 只读方法从该用户的 Core 技能图生成有界完整路径列表，普通 `node` 方法则解析 SkillPath、验证 Admin 用户共享的 Module Release、建立 Run 和运行节点目录、以批处理方式监督模块进程，并分别返回 RunId 元数据、模块 stdout、模块 stderr 与退出码。固定 Admin 用户不需要 Harness 用户记录；普通 Harness 用户只有在 `user.json` 严格有效、生命周期为 `active` 且同名用户 CLI 匹配记录的长度和 SHA-256 时才可冷启动 Host。当前 Windows 实现不监听 TCP/回环地址，只接受同一 Windows 登录会话通过本机命名管道发起的调用。
...
- **批处理技能调用**：用户 CLI 向对应 Core Host 提交一个技能调用目标与动态参数的一次性调用；一个命名管道连接只承载一个命令，模块 stdin 固定关闭。建立 Run 后 Core Host 先返回独立 RunId 元数据，再分别返回模块 stdout、模块 stderr 和退出码；用户 CLI 把 RunId 显示到自身 stderr，不建立交互会话。
...
- **技能图（Skill Map）**：固定在 `<UserHome>/map/<SkillMapId>/`、使用真实文件系统目录树保存技能帮助文档、模块选择指针和寻址索引的可查看、可修改实例。技能图目录父子关系首先表达分类、SkillPath 包含和技能子树选择范围，本身不自动构成执行顺序或显式依赖。当前只有 Core 技能图成为仓库实体；其他技能图的创建、协议和执行方式尚未实现。
- **Core 技能图**：SkillMapId 为 `core` 的内置技能图；仓库纳入 Git 的唯一默认实例及 Admin 用户当前实例固定为 `data/admin/map/core/`，普通 Harness 用户创建时把当时的 Admin Core 技能图完整复制为 `<UserHome>/map/core/` 独立快照，后续双方修改互不自动同步。当前只实现技能调用目标到 Core 技能图 SkillPath、Module Release executable 和单节点 Run 的绑定；技能子树执行、节点依赖、子树安装及运行计划尚未实现。
- **SkillPath**：一个技能节点目录相对其技能图根的规范化文件系统路径；每个路径段必须是规范小写 ASCII 文件系统安全名称。SkillPath 独立于 ModuleId、模块作者目录与资源空间中的 Resource 路径。
- **技能节点（Skill Node）**：技能图中的一个真实目录；目录包含 `skill.toml` 时该 SkillPath 可调用，不包含时只是分类节点。可调用节点可以继续包含子节点，不要求位于叶目录；技能图根本身当前不得包含 `skill.toml`。技能图根和技能节点均可包含技能帮助文档，帮助存在与否不改变节点可调用性。
...
- **技能调用目标（Skill invocation target）**：用户 CLI 提交给 Core Host 的规范相对路径，形式为 `<SkillMapId>/<SkillPath>`、`<SkillMapId>/<SkillPath>/<显式 NodeMethod 选择器>`，以及只为技能图根帮助保留的 `<SkillMapId>/.help`；当前只支持 SkillMapId `core`。省略选择器时使用隐式 `node` 方法执行目标技能节点；显式选择器由 Core Host 在最后一个路径段识别，不属于 SkillPath，也不对应技能图中的真实目录。
- **节点方法（Node method）**：Core Host 对技能调用目标提供的封闭控制面方法。当前方法集合是省略选择器时使用的 `node`，以及显式保留的 `/.tree`、`/.tree.parent-success`、`/.tree.no-structure`、`/.help` 和 `/.plan`；它们分别表示单节点执行、普通子树、强父子顺序子树、无父子顺序子树、技能树速览和计划预检，不得通过 `.node-facet`、真实点号目录或 `skill.toml` 选择 executable。当前 Core Host 实现 `node` 与 `/.help`；`/.help` 是唯一还可直接作用于技能图根的方法，其他显式保留方法必须明确报告尚未实现。
...
- **技能帮助文档（Skill help document）**：技能图根或任意技能节点目录中可选的人类维护 Markdown 文件，只提供展示信息，不声明 executable、依赖或执行顺序。当前 Core 协议允许规范文件名 `help.md`、`help.en.md`、`help.zh.md` 和 `help.zh-CN.md`；文件必须使用无 BOM 的 UTF-8，纯 ASCII 内容作为有效 UTF-8 接受。`help.md` 是默认英文和最终回退，首个非空行是 `/.help` 路径列表摘要且不得以保留的 `[run]` 标记开头。英语依次选择 `help.en.md`、`help.md`，通用中文依次选择 `help.zh.md`、`help.md`，简体中文依次选择 `help.zh-CN.md`、`help.zh.md`、`help.md`；全部缺失时节点仍有效且只显示目录名。
...
- **基础资源空间**：Core 协议保留规范名称 `author` 的作者（源代码）、`runtime` 的运行（发布）、`runs` 的运行记录和节点工作目录、`export` 和 `context` 模块专用上下文记录空间；对应根分别为 `<UserHome>/author/`、`<UserHome>/runtime/`、`<UserHome>/runs/`、`<UserHome>/export/` 和 `<UserHome>/context/`，需要时才建立。结构化日志预计归入对应 Run，但其通道和落盘协议尚未定义；`map` 不是基础资源空间。
- **RunId**：Core Host 为一次顶层技能执行生成的规范小写 32 位十六进制 UUIDv7 文本，在所属 UserHome 的 `runs` 资源空间内唯一；未来一次技能子树执行中的全部节点共享同一个 RunId，重播或重试必须建立新的 RunId 并另行引用来源 Run。
- **技能运行（Run）**：Core Host 对一次顶层技能执行建立的运行事实与工作目录集合，固定根为 `<UserHome>/runs/<RunId>/`。当前单节点执行在该根写入严格版本化 JSON `run.json`，并按 `<SkillMapId>/<SkillPath>/` 建立运行节点目录；未来子树执行继续在同一 Run 根中展开全部参与节点，不把每个节点分配成独立 Run。
- **运行节点目录（Run node directory）**：一个 Run 中对应某技能节点的目录，固定为 `<UserHome>/runs/<RunId>/<SkillMapId>/<SkillPath>/`；Core Host 在启动模块前建立它并把它作为该模块进程的当前工作目录。目录层级镜像本次运行所选技能节点的 SkillMapId 与 SkillPath，只属于该 Run 的运行事实和文件边界，不把资源空间路径反向升级为 SkillPath 声明。
...
- **REPO-006 — Harness 用户由 Admin 模块创建。** 目标中只有 Admin Core module executable 可以实现普通 Harness 用户创建与生命周期写入；当前技能调用目标 `core/admin/user/create <UserId>` 不区分调用方 Harness 用户，授权另由 Issue #55 处理。创建必须在 `data/admin/user.lock` 协调下，先完整暂存并验证 `<DataHome>/<UserId>/`，再提交 `<DataHome>/<UserId>.exe`，最后以原子文件替换把 Harness 用户记录从 `creating` 切换为 `active`；中断状态可恢复但不得被 Core Host 视作已创建用户。用户 CLI executable 与 Core Host executable 均不得自行实现该生命周期。
...
- **REPO-012 — DataHome 结构化文档格式按维护者划分。** DataHome 中由 Harness 使用者直接维护的结构化声明使用 TOML，当前具体实体为 Core 技能图中的 `skill.toml`；未来权限或检查配置只有在定义为使用者维护声明时才沿用 TOML。由程序生成或持有的事实记录与发布清单继续使用 JSON，当前具体实体包括 `user.json`、`run.json`、`swaw-harness.module.json`、Bootstrap Release 的 `manifest.json` 与模块运行状态 JSON；不得为了统一扩展名而把程序事实改作使用者配置。源码仓库内部的 Bootstrap 构建 Contract 由对应平台领域决定格式，Markdown 等非结构化文档也不属于本规则。
- **REPO-013 — 单节点 Run 由 Core Host 固化并返回身份。** Core Host 必须在验证技能声明和 Module Release 后、启动模块前生成 RunId，按需建立 `<UserHome>/runs/`、Run 根及运行节点目录，并先写入 schema 为 `swaw.harness.run/v1` 的 `run.json`。该记录必须固化 RunId、开始时间、技能调用目标、`node` 节点方法、解析后的 ModuleId、精确版本、PlatformTargetId、executable 名称、长度和 SHA-256，以及按 Windows UTF-16 code unit 无损编码的固定与动态参数；初始结果为 `running`，取得模块退出码后原子替换为 `completed` 和该退出码，Host 监督失败则原子替换为 `failed` 和错误。Run 建立后、模块启动前，Core Host 必须通过独立响应帧返回同一个 RunId；stdout、stderr 继续只通过各自调用帧返回，不构成尚未定义的结构化日志通道。
- **REPO-014 — 技能树 Help 是只读控制面。** Core Host 必须为 `<SkillMapId>/.help` 或 `<SkillMapId>/<SkillPath>/.help` 按调用惰性读取所选技能图范围，以默认深度 1 或调用方给定的 1 至 64 层后代深度，按深度优先和规范 SkillPath 顺序逐行平铺完整技能路径；技能图根行使用 `<SkillMapId>`，节点行使用 `<SkillMapId>/<SkillPath>`。单次调用最多显示 4096 个节点并最多检查 4096 个目录，后者包括技能图根、解析目标 SkillPath 经过的目录及为校验深度边界节点自身文件而识别的直接子目录；任一预算超限必须在输出任何 Help stdout 前明确失败。存在摘要或有效 `skill.toml` 可调用标记时，路径后必须追加 1 至 16 个 ASCII 空格，使 `#` 前的字符数成为 16 的倍数；`[run]` 必须位于 `#` 后且只标记可调用节点，随后显示所选语言摘要。当前语言只允许 `en`、`zh` 与 `zh-CN`，未指定时使用 `en`；帮助文件必须是有界、无 BOM 的 UTF-8 普通非 reparse 文件。Help stdout 必须使用独立 UTF-8 文本响应帧；用户 CLI 对 Windows Console 使用 Unicode 并把裸 LF 规范化为 CRLF 输出，对重定向句柄原样保留 UTF-8 字节。`/.help` 不得解析 Module Release、启动模块、建立 Run、返回 RunId 或把帮助文件升级为执行授权。
- **REPO-015 — PowerShell 源文件统一编码。** 仓库纳入 Git 的 `.ps1`、`.psm1` 与 `.psd1` PowerShell 源文件必须使用带 BOM 的 UTF-8，确保 Windows PowerShell 5.1 与现代 `pwsh` 对非 ASCII 源码作相同解码；`.editorconfig` 必须声明该编码，验证流程必须在语法解析前拒绝缺少 UTF-8 BOM 的受管文件。本规则只约束 PowerShell 源文件编码，不改变 console、pipe 或重定向输出协议。
...
- `data/admin.exe core/.help [depth] [--language en|zh|zh-CN]` 已形成不建立 Run 的技能树速览，`data/admin.exe core/admin/user/create alice` 与随后 `data/alice.exe core/helloworld [recipient]` 已形成普通 Harness 用户创建及单节点 Run 调用闭环；这些只证明帮助读取、UserHome、用户 CLI、Core Host 版本指针、Core 技能图快照、单节点运行记录和工作目录可被建立、恢复与执行。模块自身 `--help`、结构化日志、重播、资源空间 Resource 验证、Harness 用户授权、模块安装、技能组合及用户删除、升级仍未实现，不得由这些样例外推为完整系统。
