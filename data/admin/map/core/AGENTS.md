...
- **ADMIN-CORE-MAP-007 — 分类与编排共用技能图。** `data/admin/map/core/` 可以同时容纳普通分类子树和声明执行关系的编排子树；两者继续使用同一 SkillPath、技能节点和 Module Release 选择机制，不得为 Playbook 另建目录根、节点类型或发布格式。当前实例尚无子树执行策略或依赖声明实体；普通、强父子顺序和无父子顺序三种子树执行模式、显式依赖、技能子树发布及运行记录由 Issue #59 和首个真实样例确定，不得先增加空目录或宽泛占位文件。
