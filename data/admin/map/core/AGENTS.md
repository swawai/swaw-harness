...
- **ADMIN-CORE-MAP-006 — 动态参数与授权。** 调用方动态参数、资源空间及 Resource 路径如何传递，以及普通 Harness 用户选择 Admin 安装技能时如何拒绝 Admin 专用操作，留待 Core dispatcher 首次实现时确定；SkillPath 与 Resource 路径不要求相同，目录位置不得被当作授权凭据。
