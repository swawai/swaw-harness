...
- **ADMIN-DATA-002 — Admin 独占管理状态写入。** 除 Windows Bootstrap 可以从已验证 Bootstrap Release 初始物化 Module Release、Admin Core Host 版本指针与 `data/admin.exe` 外，只有 Admin Core module executable 可以安装模块、创建 Harness 用户或写入 `data/admin/` 的其他受管管理状态；Core Host executable、其他 Module Release executable 与用户 CLI executable 不得直接写入这些实体。Core Host 为 Admin 用户调用建立 `data/admin/runs/<RunId>/` 是对该用户 `runs` 资源空间的运行事实写入，不授予其修改 `map/`、`modules/`、`host/`、`user.lock` 或 Harness 用户生命周期的权限。
...
- `data/admin/map/core/` 已作为 Core 技能图实体存在；Windows Core Host 可以按技能声明执行已验证 Module Release，并为前台单节点执行建立 `data/admin/runs/<RunId>/` 运行记录和节点工作目录，但资源空间参数、结构化日志、历史 Run 操作和技能组合仍未实现。
