...
- **ADMIN-MODULE-004 — 本地版本选择。** 技能声明的精确版本、`MAJOR.*` 或 `MAJOR.MINOR.*` 只在当前 DataHome、当前 PlatformTargetId 的已验证稳定版本中选择；模糊版本选择最高匹配版本，不联网下载、不选择预发布版本，也不允许对 major 0 使用模糊版本。Core Host 版本指针只允许精确版本，并固定选择 ModuleId `swaw/core/host` 与 executable `swaw-harness-core.exe`，不得复用技能声明的模糊选择能力。
...
- **ADMIN-MODULE-006 — 版本保留。** 未再被任何 Harness 用户技能声明选择的历史版本如何枚举、保留和清理，留待出现真实磁盘回收需求后确定。

## Maintainer Notes

- Windows Bootstrap 已实现本次构建 Module Release（包括 `swaw/core/host`）的物化与清单验证，Core Host 已能按技能声明选择并执行已安装版本；Admin 的运行时模块安装技能仍未实现。
